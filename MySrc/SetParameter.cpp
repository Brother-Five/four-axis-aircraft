...
void InitParameter_Operater(void)
{
	...
	Parameter_Operater.data.PITCH_Encoder_Mid  = 2545;//一档速度(0~660)
	Parameter_Operater.data.PITCH_Encoder_Up   = 2110;//二档速度(0~660)
	Parameter_Operater.data.PITCH_Encoder_Dowm = 2700;//PITCH电机调节速度（越大越快）
	Parameter_Operater.data.YAW_Encoder_Mid    = 5724;//YAW电机调节速度（越大越快）
	Parameter_Operater.data.PITCH_6050_Offset  = 0;//6050角速度offset
	Parameter_Operater.data.YAW_6050_Offset    = 0;//6050角速度offset
	Parameter_Operater.data.RealShootSpeed     = 300;//对应射速
	Parameter_Operater.data.RealShootVolate    = 100;//对应电压影响射速	
...
//键鼠
	...
	\r\n\r\n",\
			...
			Parameter_Operater.data.RealShootSpeed,Parameter_Operater.data.RealShootVolate
			);
}
...
void ESCSetPara(float _mESC_PWM_speed,float _mESC_PWM_volate)
{
	Parameter_Operater.data.RealShootSpeed  = (uint16_t)_mESC_PWM_speed; 
	Parameter_Operater.data.RealShootVolate = (uint16_t)_mESC_PWM_volate;
	CheckParameter_Operater();
//	u1_printf("ESC_PWM_speed:%d\tESC_PWM_volate:%d\r\n",ESC_PWM_speed,ESC_PWM_volate);
}
/**
 * [ParaSetting 参数设置]
...
void bsp_ParaInit(void)
{
	...
	GetParameter_Operater();
//	{
//		extern u16 ESC_PWM_speed,ESC_PWM_volate;
//		ESC_PWM_speed = Parameter_Operater.data.RealShootSpeed;
//		ESC_PWM_volate = Parameter_Operater.data.RealShootVolate;
//	}
